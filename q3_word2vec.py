...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    probs = softmax(predicted.dot(outputVectors.T))  # 5 X 1
    cost = -np.log(probs[target])  # 1
    delta = probs  # 5 x 1
    delta[target] -= 1  # 5 x 1
    N = outputVectors.shape[0]  # 5
    D = outputVectors.shape[1]  # 3
    grad = delta.reshape(N, 1).dot(predicted.reshape(1, D))  # 5 x 3
    gradPred = outputVectors.T.dot(delta.T)  # 3 x 1
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    kidx = set()
    for i in range(0, K):
        potential_idx = dataset.sampleTokenIdx()
        while potential_idx == target or potential_idx in kidx:
            potential_idx = dataset.sampleTokenIdx()
        kidx.add(potential_idx)

    kidxvec = np.array(list(kidx))
    kvec = outputVectors.take(kidxvec, axis=0)
    uo = outputVectors[target]
    siguo = sigmoid(uo.dot(predicted))
    sigk = sigmoid(-kvec.dot(predicted))
